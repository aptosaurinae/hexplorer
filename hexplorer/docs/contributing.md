...
hexplorer is an actively maintained and utilised project.
...
[:material-bug: Report a bug](https://github.com/aptosaurinae/hexplorer/issues/new?template=BUG-REPORT.yml "Report a bug in hexplorer by creating an issue and a reproduction"){ .md-button }

__Missing information in our docs?__

[:material-file-document: Report a docs issue](https://github.com/aptosaurinae/hexplorer/issues/new?template=DOCS.yml "Report missing information or potential inconsistencies in our documentation"){ .md-button }

__Want to submit an idea?__

[:material-lightbulb-on: Request a change](https://github.com/aptosaurinae/hexplorer/issues/new?template=FEATURE-REQUEST.yml "Propose a change or feature request or suggest an improvement"){ .md-button }

__Have a question or need help?__

[:material-chat-question: Ask a question](https://github.com/aptosaurinae/hexplorer/discussions "Ask questions on our discussion board and get in touch with our community"){ .md-button }

## Developing hexplorer

To find beginner-friendly existing bugs and feature requests you may like to start out with, take a look at our [good first issues](https://github.com/aptosaurinae/hexplorer/contribute).

### Setting up a development environment

To create a development environment for hexplorer, with all libraries required for development and quality assurance installed, it is easiest to install hexplorer using the [conda](https://docs.conda.io/en/latest/) package manager, as follows:
...
1. Create the hexplorer conda environment: `conda create -n hexplorer -c conda-forge --file requirements/base.txt --file requirements/dev.txt`
1. Activate the hexplorer conda environment: `conda activate hexplorer`
1. Install the hexplorer package into the environment, in editable mode and ignoring dependencies (we have dealt with those when creating the conda environment): `pip install --no-deps -e .`
...
Either way, you should add your environment as a jupyter kernel, so the example notebooks can run in the tests: `ipython kernel install --user --name=hexplorer`
...
!!! note
    ...
    If you already have an environment called `hexplorer` on your system (e.g., for a stable installation of the package), you will need to [chose a different environment name][choosing-a-different-environment-name].
...
hexplorer can be memory intensive; we like to ensure that any development to the core code does not exacerbate this.
...
1. Install [memray](https://bloomberg.github.io/memray/index.html) in your `hexplorer` conda environment: `conda install memray pytest-memray`.
...
??? question "I have a new example notebook"
    ...
    If the project is set up to include process examples (see `.copier-answers.yaml` option `create_jupyter_notebook_directory`),
...
This project has been built with [copier](https://copier.readthedocs.io/) based on the [Arup Python project template](https://github.com/arup-group/pypackage-template).
When changes are made to the base template, they can be merged into this project by running `copier update --skip-answered` from the  `hexplorer` conda environment.
